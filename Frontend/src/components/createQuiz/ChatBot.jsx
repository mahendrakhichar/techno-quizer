...
import { Bot } from 'lucide-react'; // Replacing Robot with PaperPlaneRight (icon for sending messages)
...
const ChatBot = () => {
    ...
    const searchHandler = async (e) => {
        ...
        if (userSearch.trim() === "") return; // Prevent empty questions

        setLoading(true); // Show loading animation while fetching answer
        try {
            const response = await axios.post('/api/userQuestion/gpt', { question: userSearch });
            setGptAnswer(response.data.message);  // Assuming the response contains the answer in `response.data`
        } catch (err) {
    ...
    return (
        <div className="bg-gradient-to-r from-purple-600 via-indigo-500 to-blue-400 p-10 rounded-xl shadow-lg w-full mt-8">
            ...
            <form onSubmit={searchHandler} className="space-y-6">
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className={`px-4 py-3 rounded-lg text-white ${loading ? 'bg-gray-400' : 'bg-indigo-600 hover:bg-indigo-700'} focus:outline-none`}
                    >
